...
class ClientToMasterFragment : BaseFragment(R.layout.fragment_client_to_master) {

    private val binding by viewBinding { FragmentClientToMasterBinding.bind(it) }
        ...
        binding.btnSignUp.setOnClickListener {
            with(binding) {
                if (etMasterFullname.text.isNotEmpty()
                    ...
                    val clientToMasterRequest = ClientToMasterRequest(
                        fullName = fullName,
...
//                    viewModel.masterRegister(masterRegister)
                    clientProfileViewModel.clientToMaster(clientToMasterRequest)
    ...
    private fun observeViewModel() {
        ...
        viewLifecycleOwner.lifecycleScope.launchWhenCreated {
            clientProfileViewModel.clientToMaster.collect {
                when(it) {
                    is UiStateObject.LOADING -> {
                        showLoading()
                    }
    ...
    private fun setupClientProfileViewModel() {
        val token = SharedPref(requireContext()).getString(KEY_ACCESS_TOKEN)
        clientProfileViewModel = ViewModelProvider(
            this,
